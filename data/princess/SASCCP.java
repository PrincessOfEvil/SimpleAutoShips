...
public class SASCCP extends BaseCampaignPlugin
	{
	private static Logger logger = Global.getLogger(SASCCP.class);
	...
	public boolean hackPluginActive()
		{
		...
		if (sector != null)
			...
			if (player != null)
				return player.getStats().getSkillLevel("automated_ships") <= 0.0f;
			}
		return false;
		}
	public boolean crackPluginActive()
		{
		SectorAPI sector = Global.getSector();
		if (sector != null)
			{
			CampaignUIAPI ui = sector.getCampaignUI();
			if (ui instanceof CampaignState)
				{
				CampaignState state = (CampaignState)ui;
				return state.getDialogType() != null;
				}
			}
		return false;
		}
	...
	public PluginPick<AICoreOfficerPlugin> pickAICoreOfficerPlugin(String commodityId) {
		if (hackPluginActive() && crackPluginActive() &&
			("omega_core".equals(commodityId) || 
			"alpha_core".equals(commodityId) || 
			"beta_core".equals(commodityId) || 
			"gamma_core".equals(commodityId)))
			return new PluginPick(null, CampaignPlugin.PickPriority.MOD_SPECIFIC); 
		return null;
		}
	}